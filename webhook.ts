...
import type { ClaimRequest, WebhookEmbed, WebhookField, WebhookPayload, WebhookResult } from "./types";

const SUCCESS_COLOR = 0x43b581;
const FAILURE_COLOR = 0xf04747;
const TEST_COLOR = 0x5865f2;
const CAPTCHA_DETECTED_COLOR = 0xfaa61a;
const CAPTCHA_CLEARED_COLOR = 0x43b581;
...
function buildUserProfileUrl(userId?: string): string | null {
    return userId ? `https://discord.com/users/${userId}` : null;
}

function buildMessageUrl(request: ClaimRequest): string | null {
...
function buildMessageField(request: ClaimRequest): WebhookField | null {
    ...
    return { name: "Message", value: `[Posted here!](${messageUrl})`, inline: false };
}

function buildClaimFields(request: ClaimRequest, giftType: string | null): WebhookField[] {
    return [
...
function getResultPresentation(result: WebhookResult) {
    switch (result) {
        case "claimed":
            return { title: "🎉 Yay! Claimed a Nitro!", color: SUCCESS_COLOR };
        case "failed":
        default:
            return { title: "❌ Failed to claim nitro", color: FAILURE_COLOR };
    }
}
...
function buildEmbedAuthor(request: ClaimRequest) {
    ...
    return { name, icon_url: request.authorAvatarUrl };
}
...
function buildClaimWebhookPayload(
    ...
    return createPayload([buildClaimEmbed(result, request, giftType)]);
}

function buildCaptchaEmbed(status: "detected" | "cleared"): WebhookEmbed {
    return status === "detected"
        ? {
            title: "⚠️ hCaptcha Detected",
            description: "An hCaptcha challenge is currently visible on screen. Please wait for a refresh.",
            color: CAPTCHA_DETECTED_COLOR,
            timestamp: new Date().toISOString(),
            footer: { text: WEBHOOK_NAME },
        }
        : {
            title: "✅ hCaptcha Cleared",
            description: "The hCaptcha challenge is no longer visible.",
            color: CAPTCHA_CLEARED_COLOR,
            timestamp: new Date().toISOString(),
            footer: { text: WEBHOOK_NAME },
        };
}

function buildCaptchaWebhookPayload(status: "detected" | "cleared"): WebhookPayload {
    return createPayload([buildCaptchaEmbed(status)]);
}

function parseWebhookError(data: string, status: number): string {
    if (!data) return `Webhook request failed with status ${status}.`;
    try {
        const body = JSON.parse(data) as { message?: string; errors?: unknown };
        const detail = body.message ?? (body.errors ? JSON.stringify(body.errors) : null);
        return detail
...
export async function sendCaptchaStatusWebhook(
    webhookUrl: string,
    status: "detected" | "cleared"
) {
    const url = parseWebhookUrl(webhookUrl);
    if (!url) return;
    await postWebhook(url, buildCaptchaWebhookPayload(status));
}
